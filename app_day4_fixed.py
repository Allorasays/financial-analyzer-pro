#!/usr/bin/env python3
"""
Financial Analyzer Pro - Day 4: Portfolio Management (Fixed)
...
def main():
    ...
    if page == "💼 Portfolio Management":
        ...
        st.info("Market news integration coming in future updates")
        ...
        with col1:
            lookup_symbol = st.text_input("Enter Stock Symbol", value="AAPL", key="market_lookup")
        with col2:
            if st.button("Lookup", key="market_lookup_btn"):
                if lookup_symbol:
                    with st.spinner(f"Looking up {lookup_symbol}..."):
                        data, error = get_stock_data(lookup_symbol, "1d")
                        if data is not None and not data.empty:
                            current_price = data['Close'].iloc[-1]
                            previous_price = data['Open'].iloc[-1] if len(data) > 0 else current_price
                            change = current_price - previous_price
                            ...
                            st.success(f"**{lookup_symbol.upper()}**: ${current_price:.2f} ({change_percent:+.2f}%)")
                        else:
