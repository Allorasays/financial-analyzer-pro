#!/usr/bin/env python3
"""
Financial Analyzer Pro with Real-Time Features
Includes portfolio management, technical analysis, ML predictions, and real-time data
"""
...
# Real-time imports with graceful fallbacks
try:
    from realtime_simple import get_live_price, get_market_overview, get_stock_data, get_trending_stocks
    from realtime_dashboard_simple import (
...
# Page config
st.set_page_config(
    page_title="Financial Analyzer Pro - Real-Time",
    page_icon="📊",
    layout="wide",
...
def main():
    ...
    st.title("📊 Financial Analyzer Pro - Real-Time")
    st.markdown("**Advanced Financial Analysis with Live Data & Portfolio Management**")
    st.markdown('</div>', unsafe_allow_html=True)
    
    # Real-time mode indicator
    if st.session_state.realtime_mode:
        st.markdown('<div class="realtime-indicator">🔴 REAL-TIME MODE ACTIVE</div>', unsafe_allow_html=True)
    ...
    if REALTIME_AVAILABLE:
        realtime_mode = st.sidebar.checkbox("🔴 Real-Time Mode", value=st.session_state.realtime_mode, help="Enable live data updates")
        if realtime_mode != st.session_state.realtime_mode:
    ...
    analysis_type = st.sidebar.selectbox(
        "Analysis Type",
        ["📈 Stock Analysis", "💼 Portfolio Management", "📊 Market Overview", "🤖 ML Predictions", "🔴 Real-Time Data"],
        index=0
    )
    ...
    if analysis_type == "📈 Stock Analysis":
        ...
        if REALTIME_AVAILABLE and st.session_state.realtime_mode:
            display_realtime_market_overview()
        else:
        ...
        if not REALTIME_AVAILABLE:
            ...
            with col1:
                realtime_mode = st.checkbox("🔴 Enable Real-Time Mode", value=st.session_state.realtime_mode, help="Enable live data streaming and auto-refresh")
                if realtime_mode != st.session_state.realtime_mode:
                    st.session_state.realtime_mode = realtime_mode
                    st.rerun()
            with col2:
    ...
    if st.session_state.realtime_mode and REALTIME_AVAILABLE:
        time.sleep(30)  # Wait 30 seconds
        st.rerun()
    
    # Footer
    st.markdown("---")
    st.markdown("**Financial Analyzer Pro - Real-Time** | Built with Streamlit")
    st.markdown("*Advanced financial analysis with live data, portfolio management, and ML predictions*")
